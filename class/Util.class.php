...
{	
	...
	static function wxapp_code_path_convert($url,$size) {
		...
		if ($url) {
			...
			Image::create($url)->resize($size, $size)->saveTo(ATTACHMENT_ROOT . $path . $filename);
	...
	static function ihttp_email($to, $subject, $body, $global = false) {
		global $_W;
		...
		if (empty($mailer)) {
			...
			$set = pdo_get('zofui_sitetemp_smtp');
			
			if( empty( $set ) ) return array('status'=>201,'res'=>'还没有填写邮件参数');
			...
			if ($set['type'] == 0) {
				...
				return array('status'=>201,'res'=>'不支持的SMTP服务器');
			}
			
			
			/*if (!empty($config['smtp']['authmode'])) {
				if (!extension_loaded('openssl')) {
					return array('status'=>201,'res'=>'请开启 php_openssl 扩展！');
				}
			}*/
			$mailer->signature = $set['sign'];
			$mailer->isSMTP();
			...
			$mailer->Username = $set['account'];
			$mailer->Password = $set['pass'];
			//!empty($config['smtp']['authmode']) && $mailer->SMTPSecure = 'ssl';

			$mailer->From = $set['account'];
			$mailer->FromName = $set['name'];
			$mailer->isHTML(true);
		}
				...
				foreach($body as $value) {
					if (substr($value, 0, 1) == '@') {
						if(!is_file($file = ltrim($value, '@'))){
							return array('status'=>201,'res'=>'附件不存在或非文件！');
						}
		...
		if (!empty($mailer->signature)) {
			$newbody .= htmlspecialchars_decode($mailer->signature);
		}
		...
		if ($mailer->send()) {
			return array('status'=>200,'res'=>'已发送');
		} else {
			return array('status'=>201,'res'=>$mailer->ErrorInfo);
		}
	}
	...
	static function  shortUrl($url){
		...
		if( empty( $cache ) ){
			...
			$url = 'https://goo.gd/action/json.php?source=1681459862&url_long='. urlencode( $url );
			$urlres = json_decode(self::httpGet($url),true);
	...
	// 腾讯位置转百度位置
	function tencentToBaidu($lat,$lng){
	...
	static function getarea($ip = ''){
		...
		if(empty($res)) {
			$res = self::httpGet('http://int.dpool.sina.com.cn/iplookup/iplookup.php?format=js&ip=' . $ip);
		}
	...
	// 查询最新的一个
	static function getNew($tablename,$where,$select = ' * '){
		global $_W;
		$data = self::structWhereStringOfAnd($where);
		$sql = "SELECT $select FROM ". tablename($tablename) ." WHERE $data[0]  ORDER BY `id` DESC ";
	...
	//在一个表里查询多条数据
	static function getAllDataInSingleTable($tablename,$where,$page,$num,$order='id DESC',$iscache = false,$isNeedPager = true,$select = '*',$str='',$cachename='p',$isuni=true){
		global $_W;
		$data = self::structWhereStringOfAnd($where,'',$isuni);
	...
	*/
	...
	//组合AND数据查询where字符串 = ,>= ,<= <、>必须紧挨字符 例：$where = array('status'=>1,'overtime<'=>time());
 	static function structWhereStringOfAnd($array,$head='',$isuni=true){
		global $_W;
		if(!is_array($array)) return false;
		if( $isuni ) $array['uniacid'] = $_W['uniacid'];
	...
	//查询缓存
 	static function getCache($key,$name,$isuni = true) {
		global $_W;
		if(empty($key) || empty($name)) return false;

		$uniacid = $isuni ? $_W['uniacid'].':' : '';

		return cache_read('zfst:'.$uniacid.$key.':'.$name);
	}
	
	//设置缓存
	static function setCache($key,$name,$value,$isuni = true) {
		global $_W;
		if(empty($key) || empty($name)) return false;	

		$uniacid = $isuni ? $_W['uniacid'].':' : '';

		$res = cache_write('zfst:'.$uniacid.$key.':'.$name,$value);
		return $res;
	}
	
	//删除缓存
	static function deleteCache($key,$name,$isuni = true) {
		global $_W;		
		if(empty($key) || empty($name)) return false;

		$uniacid = $isuni ? $_W['uniacid'].':' : '';
		cache_write('zfst:'.$uniacid.$key.':'.$name,'');

		return cache_delete('zfst:'.$uniacid.$key.':'.$name);
	}
	...
	static function register_jssdk($url='',$debug = false){
		global $_W;
		if (defined('HEADER')) {
			echo '';
			return;
		...
		if( !empty( $sitearr ) ) {
			...
			foreach ($sitearr as $v) {
				if( $_W['siteroot'] == $v['site1'] || $_W['siteroot'] == $v['site2'] && $_W['acid'] != $v['siteacid'] ){
					$account_api = WeAccount::create( $v['siteacid'] );
...
//./resource/components/ueditor/ueditor.all.min.js
//../addons/zofui_countprice/admin/public/ueditor.all.js
static function tpl_ueditor($id, $value = '', $options = array()) {
	...
	<script type=\"text/javascript\">
			var ueditoroption = {
				...
				'toolbars' : [['fullscreen', 'source', 'preview', '|', 'bold', 'italic', 'underline', 'strikethrough', 'forecolor', 'backcolor', '|',
					'justifyleft', 'justifycenter', 'justifyright', '|', 'insertorderedlist', 'insertunorderedlist', 'blockquote', 'removeformat', '|', 'rowspacingtop', 'rowspacingbottom', 'lineheight','indent', 'paragraph', 'fontsize']],
			...
			UE.registerUI('myinsertimage',function(editor,uiName){
				editor.registerCommand(uiName, {
					...
					util.image($('#mmm'+editor.ui.id), function(url){
						for( var i = 0;i < url.length;i++){
							editor.execCommand('insertimage', {
								'src' : url[i].url,
								'_src' : url[i].attachment,
								'width' : '100%',
								'alt' : url[i].filename
							});
						}
	...
	<div class="input-group">
		...
		<a class="btn btn-default js-image-up js-image-{$name}">上传图片</a>
	</div>
