...
class Zofui_sitetempModule extends WeModule {
	...
	public function settingsDisplay($settings) {
		...
		if(!pdo_fieldexists('zofui_sitetemp_page', 'isinit')) {
		    pdo_query("ALTER TABLE ".tablename('zofui_sitetemp_page')." ADD `isinit` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '1添加页面 0无';");
		}

	  	if(!pdo_fieldexists('zofui_sitetemp_article', 'vurl')) {
	    	pdo_query("ALTER TABLE ".tablename('zofui_sitetemp_article')." ADD `vurl` varchar(600) DEFAULT NULL;");
	  	}

	  	if(!pdo_fieldexists('zofui_sitetemp_product', 'vurl')) {
	    	pdo_query("ALTER TABLE ".tablename('zofui_sitetemp_product')." ADD `vurl` varchar(600) DEFAULT NULL;");
	  	}

	  	if(!pdo_fieldexists('zofui_sitetemp_article', 'falseview')) {
	    	pdo_query("ALTER TABLE ".tablename('zofui_sitetemp_article')." ADD `falseview` int(11) unsigned NOT NULL DEFAULT '0';");
	  	}


		header('Location: ./index.php?c=site&a=entry&do=temp&p=site&op=my&m=zofui_sitetemp');die;
